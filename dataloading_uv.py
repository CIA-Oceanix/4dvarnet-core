...
class FourDVarNetDataset(Dataset):
    ...
    def __init__(
        ...
        self.return_coords = False
        self.pp=pp
        if ais_datas is not None:
            self.ais_datas = ais_datas[ais_index]
	...
	###
	#Add ais dat to data loader
	###
	
        ais_dat = self.ais_datas
	...
	###todo modifier juste essai
        ...
        if self.sst_ds == None:
            return oi_item, obs_mask_item, obs_item, gt_item, u_item, v_item, lat_item, lon_item
        else:
...
class FourDVarNetDataModule(pl.LightningDataModule):
    ...
    def mean_stds(self, ds):
        ...
        if self.sst_var == None:
            return [mean, std], [mean_uv, std_uv]
        else:
            print('... Use SST data')
    ...
    def compute_norm_stats(self, ds):
        """
        If sst is used, return norm_stats and norm_stats_sst (and
        norm_stats_uv if `u_var` is specified).

        Otherwise, return norm_stats and norm_stats_uv.
        """
        if self.pp == 'std':
    ...
    def setup(self, stage=None):
        ...
        ais_dataset_train = torch.cat([border_x, ais_dataset_train,border_x], dim = 3)
        ais_dataset_train = torch.cat([border_y, ais_dataset_train,border_y], dim = 4)
        
        ais_dataset_train = ais_dataset_train[:,None,:]
        ais_dataset_train = torch.cat([0*ais_dataset_train[:3],ais_dataset_train,0*ais_dataset_train[:3]])
        
        dT = 7
        
        windowed_dataset = torch.tensor(ais_dataset_train[:-7])
        print(windowed_dataset.size(),ais_dataset_train.size())
        for i in range (dT-1):
        	
            windowed_dataset = torch.cat([windowed_dataset, ais_dataset_train[i + 1 : len(ais_dataset_train) -dT + i + 1 ]],dim = 1)
            
        print("l 769 dataloader" , windowed_dataset.size())
        self.ais_dataset_train = windowed_dataset
        
        
        
        
        print("ligne 759 dataloader", self.ais_dataset_train.size())
        ...
        print("taille du dataset : ", len(self.train_ds))
        ...
        count_val = 0
        for sl in self.val_slices:
        
        	list_dataset_val.append(
        	FourDVarNetDataset(
                    dim_range={**self.dim_range, **{'time': sl}},
                    strides=self.strides,
                    slice_win=self.slice_win,
        ...
        print(" ")
        print("taille des dataloader : ")
        print("train : ", len(self.train_ds)," val : ", len(self.val_ds)," test : ", len(self.test_ds))
        print(" ")
        
        
        print(self.train_ds)
        #self.ais_dataset_val = torch.zeros((466,10,10))
        #self.ais_dataset_test = torch.zeros((466,10,10))
        
        if self.sst_var is None:
            self.norm_stats, self.norm_stats_uv = self.compute_norm_stats(self.train_ds)
            self.set_norm_stats(self.train_ds, self.norm_stats, ns_uv=self.norm_stats_uv)
            self.set_norm_stats(self.val_ds, self.norm_stats, ns_uv=self.norm_stats_uv)
            self.set_norm_stats(self.test_ds, self.norm_stats, ns_uv=self.norm_stats_uv)
        else:
