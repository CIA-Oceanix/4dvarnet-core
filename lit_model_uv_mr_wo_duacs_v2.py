...
class LitModelUV(pl.LightningModule):
        ...
        for _iter in range(self.hparams.n_fourdvar_iter):
            if ( phase == 'test' ) & ( self.use_sst ): 
                ...
                if self.hparams.n_fourdvar_iter_hr > 0 :
                    ...
                    _loss_hr = 0.
                    state_hr = [None]
                    out_hr = [ torch.nn.functional.interpolate(out_lr[0][:,_dt:_dt+self.hparams.dT_hr_model,:,:].detach(), scale_factor=self.scale_lr, mode='bicubic') ,
                              torch.nn.functional.interpolate(out_lr[1][:,_dt:_dt+self.hparams.dT_hr_model,:,:].detach(), scale_factor=self.scale_lr, mode='bicubic') ,
    ...
    def build_test_xr_ds_sst(self, outputs, diag_ds):
        ...
        if True :
            ...
            w = np.zeros_like( self.patch_weight_hr.detach().cpu().numpy() )
            w[int(self.hparams.dT_hr_model/2),:,:] = 1.
    ...
    def get_init_state_lr_from_hr(self, batch,out_hr=(None,),out_lr=(None,),mask_sampling = None):
        ...
        if out_hr[0] is not None:            
            ...
            init_ssh_ = self.patch_weight_lr * init_ssh_ + (1.-self.patch_weight_hr) * out_lr[0][:,_dt:_dt+self.hparams.dT_hr_model,:,:] 
            init_u_   = self.patch_weight_lr *  init_u_  + (1.-self.patch_weight_hr) * out_lr[1][:,_dt:_dt+self.hparams.dT_hr_model,:,:]
            init_v_   = self.patch_weight_lr *  init_v_  + (1.-self.patch_weight_hr) * out_lr[2][:,_dt:_dt+self.hparams.dT_hr_model,:,:]
