...
import os
import sys
sys.path.append('/home/clement/4dvarnet-core/ose/eval_notebooks')
import logging
import pandas as pd
...
data_baseline = xr.open_dataset(file).update({
                     'obs_inp':(('time','lat','lon'),data_4dvarnet.gt.values),
                     'pred':(('time','lat','lon'),data_4dvarnet.oi.values)})
data_4dvarnet = xr.open_dataset(file).update({
