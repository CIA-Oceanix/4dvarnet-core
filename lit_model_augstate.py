""" Lit model for interpolation with DUACS as anomaly """
...
def get_4dvarnet(hparams):
    return NN_4DVar.Solver_Grad_4DVarNN(
        Phi_r(hparams.shape_state[0], hparams.DimAE, hparams.dW, hparams.dW2,
              hparams.sS, hparams.nbBlocks, hparams.dropout_phi_r,
              hparams.stochastic), Model_H(hparams.shape_state[0]),
...
def get_phi(hparams):
    ...
    class PhiPassThrough(torch.nn.Module):
        ...
        def __init__(self):
            super().__init__()
            self.phi = Phi_r(hparams.shape_data[0], hparams.DimAE, hparams.dW,
...
def get_constant_crop(patch_size, crop, dim_order=['time', 'lat', 'lon']):
    patch_weight = np.zeros([patch_size[d] for d in dim_order],
                            dtype='float32')
...
class LitModelAugstate(pl.LightningModule):
    ...
    def __init__(self,
        ...
        hparams = hparam if isinstance(
            hparam, dict) else OmegaConf.to_container(hparam, resolve=True)
        ...
        self.var_Val = self.hparams.var_Val if hasattr(self.hparams,
                                                       'var_Val') else None
        self.var_Tr = self.hparams.var_Tr if hasattr(self.hparams,
                                                     'var_Tr') else None
        self.var_Tt = self.hparams.var_Tt if hasattr(self.hparams,
                                                     'var_Tt') else None
        self.mean_Val = self.hparams.mean_Val if hasattr(
            self.hparams, 'mean_Val') else None
        self.mean_Tr = self.hparams.mean_Tr if hasattr(self.hparams,
                                                       'mean_Tr') else None
        self.mean_Tt = self.hparams.mean_Tt if hasattr(self.hparams,
                                                       'mean_Tt') else None
        ...
        self.patch_weight = torch.nn.Parameter(torch.from_numpy(
            call(self.hparams.patch_weight)),
                                               requires_grad=False)

        # main model

        self.model_name = self.hparams.model if hasattr(
            self.hparams, 'model') else '4dvarnet'
        self.use_sst = self.hparams.sst if hasattr(self.hparams,
                                                   'sst') else False
        self.aug_state = self.hparams.aug_state if hasattr(
            self.hparams, 'aug_state') else False
        ...
        self.w_loss = torch.nn.Parameter(
            ...
            requires_grad=False)  # duplicate for automatic upload to gpu
        ...
        self.tr_loss_hist = []
        self.automatic_optimization = self.hparams.automatic_optimization if hasattr(
            self.hparams, 'automatic_optimization') else False

        self.median_filter_width = self.hparams.median_filter_width if hasattr(
            self.hparams, 'median_filter_width') else 1
    ...
    def on_train_epoch_start(self):
        if self.model_name in ('4dvarnet', '4dvarnet_sst'):
            ...
            if (self.current_epoch
                    in self.hparams.iter_update) & (self.current_epoch > 0):
                indx = self.hparams.iter_update.index(self.current_epoch)
                print(
                    '... Update Iterations number/learning rate #%d: NGrad = %d -- lr = %f'
    ...
    def training_epoch_end(self, outputs):
        ...
        if len(best_ckpt_path) > 0:
            ...
            if should_reload_ckpt(torch.stack([out['loss']
                                               for out in outputs])):
    ...
    def training_step(self, train_batch, batch_idx, optimizer_idx=0):
        ...
        losses, _, metric = self(train_batch, phase='train')
        if losses[-1] is None:
            print("None loss")
            return None
    ...
    def build_test_xr_ds(self, outputs, diag_ds):
        ...
        for ds in dses:
            ...
            xr_weight = xr.DataArray(self.patch_weight.detach().cpu(),
                                     ds.coords,
                                     dims=ds.gt.dims)
            _ds = ds.pipe(lambda dds: dds * xr_weight).assign(
                weight=xr_weight).broadcast_like(fin_ds).fillna(0.).where(
                    ds_nans == 0, np.nan)
            fin_ds = fin_ds + _ds

        return (
            (fin_ds.drop('weight') / fin_ds.weight).sel(
                instantiate(self.test_domain)).isel(
                    time=slice(self.hparams.dT // 2, -self.hparams.dT // 2))
            # .pipe(lambda ds: ds.sel(time=~(np.isnan(ds.gt).all('lat').all('lon'))))
        ).transpose('time', 'lat', 'lon')

    def nrmse_fn(self, pred, ref, gt):
        return (self.test_xr_ds[[
            pred, ref
        ]].pipe(lambda ds: ds - ds.mean()).pipe(lambda ds: ds - (
            self.test_xr_ds[gt].pipe(lambda da: da - da.mean()))).pipe(
                lambda ds: ds**2 / self.test_xr_ds[gt].std()).to_dataframe().
                pipe(lambda ds: np.sqrt(ds.mean())).to_frame().rename(
                    columns={
                        0: 'nrmse'
    ...
    def mse_fn(self, pred, ref, gt):
        return (self.test_xr_ds[[
            pred, ref
        ]].pipe(lambda ds: ds - self.test_xr_ds[gt]).pipe(lambda ds: ds**2).
                to_dataframe().pipe(lambda ds: ds.mean()).to_frame().rename(
                    columns={
                        0: 'mse'
    ...
    def sla_diag(self, t_idx=3, log_pref='test'):
        ...
        t_idx = 3
        fig_maps = plot_maps(self.x_gt[t_idx], self.obs_inp[t_idx],
                             self.x_oi[t_idx], self.x_rec[t_idx],
                             self.test_lon, self.test_lat, path_save0)
        path_save01 = self.logger.log_dir + '/maps_Grad.png'
        fig_maps_grad = plot_maps(self.x_gt[t_idx],
                                  self.obs_inp[t_idx],
                                  self.x_oi[t_idx],
                                  self.x_rec[t_idx],
                                  self.test_lon,
        ...
        nrmse_fig = plot_nrmse(self.x_gt,
                               self.x_oi,
                               self.x_rec,
                               path_save3,
                               time=self.test_dates)
        self.test_figs['nrmse'] = nrmse_fig
        self.logger.experiment.add_figure(f'{log_pref} NRMSE',
                                          nrmse_fig,
                                          global_step=self.current_epoch)
        # plot SNR
        ...
        self.logger.experiment.add_figure(f'{log_pref} SNR',
                                          snr_fig,
                                          global_step=self.current_epoch)
        psd_ds, lamb_x, lamb_t = metrics.psd_based_scores(
            self.test_xr_ds.pred, self.test_xr_ds.gt)
        fig, spatial_res_model, spatial_res_oi = get_psd_score(
            self.test_xr_ds.gt,
            self.test_xr_ds.pred,
            self.test_xr_ds.oi,
            with_fig=True)
        self.test_figs['res'] = fig
        self.logger.experiment.add_figure(f'{log_pref} Spat. Resol',
                                          fig,
                                          global_step=self.current_epoch)
        psd_ds, lamb_x, lamb_t = metrics.psd_based_scores(
            self.test_xr_ds.pred, self.test_xr_ds.gt)
        ...
        self.logger.experiment.add_figure(f'{log_pref} PSD',
                                          psd_fig,
                                          global_step=self.current_epoch)
        _, _, mu, sig = metrics.rmse_based_scores(self.test_xr_ds.pred,
                                                  self.test_xr_ds.gt)

        mdf = pd.concat([
            nrmse_df.rename(
                columns=lambda c: f'{log_pref}_{c}_glob').loc['pred'].T,
            mse_df.rename(
                columns=lambda c: f'{log_pref}_{c}_glob').loc['pred'].T,
        ])
        md = {
            f'{log_pref}_spatial_res':
            float(spatial_res_model),
            f'{log_pref}_spatial_res_imp':
            float(spatial_res_model / spatial_res_oi),
            f'{log_pref}_lambda_x':
            lamb_x,
            f'{log_pref}_lambda_t':
            lamb_t,
            f'{log_pref}_mu':
            mu,
            f'{log_pref}_sigma':
    ...
    def teardown(self, stage='test'):
        ...
        self.logger.log_hyperparams({**self.hparams}, self.latest_metrics)
    ...
    def get_init_state(self, batch, state=(None, )):
        ...
        if self.aug_state:
            ...
            init_state = torch.cat(
                (targets_OI, inputs_Mask * (inputs_obs - targets_OI)), dim=1)
        return init_state
    ...
    def sla_loss(self, gt, out):
        ...
        loss = NN_4DVar.compute_spatio_temp_weighted_loss((out - gt),
                                                          self.patch_weight)
        loss_grad = (
    ...
    def compute_loss(self, batch, phase, state_init=(None, )):
        ...
        obs = torch.cat((targets_OI, inputs_Mask * (inputs_obs - targets_OI)),
                        dim=1)
        new_masks = torch.cat((torch.ones_like(inputs_Mask), inputs_Mask),
                              dim=1)
        ...
        if self.aug_state:
            ...
            new_masks = torch.cat((new_masks, torch.zeros_like(inputs_Mask)),
                                  dim=1)
        ...
        with torch.set_grad_enabled(True):
            ...
            if self.aug_state:
                ...
                outputs = outputsSLR + outputs[:, self.hparams.dT:2 *
                                               self.hparams.dT, :, :]
            ...
            if self.median_filter_width > 1:
                outputs = kornia.filters.median_blur(
                    outputs,
                    (self.median_filter_width, self.median_filter_width))

            # reconstruction losses
            # projection losses

            yGT = torch.cat((targets_OI, targets_GT_wo_nan - outputsSLR),
                            dim=1)
            if self.aug_state:
            ...
            metrics_dict = dict([('mse', mse), ('mseGrad', mseGrad),
                                 ('meanGrad', mean_GAll),
                                 ('mseOI', loss_OI.detach()),
                                 ('mseGOI', loss_GOI.detach())])

        return loss, outputs, [outputsSLRHR, hidden_new, cell_new,
...
if __name__ == '__main__':
    ...
    from omegaconf import OmegaConf
    OmegaConf.register_new_resolver("mul",
                                    lambda x, y: int(x) * y,
                                    replace=True)
    import hydra_config
    ...
    mod = get_model(cfg_n, ckpt, dm=dm)
    mod.hparams
    cfg = get_cfg(cfg_n)
