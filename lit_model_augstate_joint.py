...
class LitModelAugstate(pl.LightningModule):
    ...
    def diag_step(self, batch, batch_idx, log_pref='test'):
        targets_OI, inputs_Mask, inputs_obs, targets_GT, sst_gt, mask_sst = batch
        ...
        if loss is not None:
            ...
            self.log(f'{log_pref}_mse_ssh', metrics[-1]["mseSSH"] / self.var_Tt, on_step=False, on_epoch=True, prog_bar=True)
            self.log(f'{log_pref}_mse_sst', metrics[-1]["mseSST"] / self.var_Tt, on_step=False, on_epoch=True, prog_bar=True)
            self.log(f'{log_pref}_mse_gssh', metrics[-1]['mseGrad'] / metrics[-1]['meanGrad'], on_step=False, on_epoch=True, prog_bar=True)

        return {'ssh_gt'    : (targets_GT.detach().cpu() * np.sqrt(self.var_ssh_Tr)) + self.mean_ssh_Tr,
                'ssh_oi'    : (targets_OI.detach().cpu() * np.sqrt(self.var_ssh_Tr)) + self.mean_ssh_Tr,
                'ssh_obs'    : (inputs_obs.detach().where(inputs_Mask, torch.full_like(inputs_obs, np.nan)).cpu() * np.sqrt(self.var_ssh_Tr)) + self.mean_ssh_Tr,
                'ssh_pred' : (out[0].detach().cpu() * np.sqrt(self.var_ssh_Tr)) + self.mean_ssh_Tr,
                'sst_gt' : (sst_gt.detach().cpu() * np.sqrt(self.var_sst_Tr)) + self.mean_sst_Tr,
                'sst_obs'    : (sst_obs.detach().where(mask_sst, torch.full_like(mask_sst, np.nan)).cpu() * np.sqrt(self.var_sst_Tr)) + self.mean_sst_Tr,
                'sst_pred' : (out[1].detach().cpu() * np.sqrt(self.var_sst_Tr)) + self.mean_sst_Tr,
                }
    ...
    def sla_diag(self, t_idx=3, log_pref='test'):
        ...
        if self.hparams.animate == True:
            ...
            animate_maps(self.x_ssh_gt, self.obs_ssh_inp, self.x_oi, self.x_ssh_rec, self.lon, self.lat, path_save0)
            # save NetCDF
        ...
        # compute SSH scores
        nrmse_df = self.nrmse_fn('ssh_pred', 'ssh_oi', 'ssh_gt')
        mse_df = self.mse_fn('ssh_pred', 'ssh_oi', 'ssh_gt')
        ...
        nrmse_fig = plot_nrmse(self.x_ssh_gt,  self.x_ssh_oi, self.x_ssh_rec, path_save3, time=self.test_dates)
        ...
        snr_fig = plot_snr(self.x_ssh_gt, self.x_ssh_oi, self.x_ssh_rec, path_save4)
        self.test_figs['snr'] = snr_fig

        self.logger.experiment.add_figure(f'{log_pref} SNR', snr_fig, global_step=self.current_epoch)
        psd_ds_ssh, lamb_x_ssh, lamb_t_ssh = metrics.psd_based_scores(self.test_xr_ds.ssh_pred, self.test_xr_ds.ssh_gt)
        fig, spatial_res_model_ssh, spatial_res_oi_ssh = get_psd_score(self.test_xr_ds.ssh_gt, self.test_xr_ds.ssh_pred, self.test_xr_ds.ssh_oi, with_fig=True)
        self.test_figs['res'] = fig
        self.logger.experiment.add_figure(f'{log_pref} Spat. Resol', fig, global_step=self.current_epoch)
        psd_ds_ssh, lamb_x_ssh, lamb_t_ssh = metrics.psd_based_scores(self.test_xr_ds.ssh_pred, self.test_xr_ds.ssh_gt)
        psd_fig = metrics.plot_psd_score(psd_ds_ssh)
        ...
        self.logger.experiment.add_figure(f'{log_pref} SNR SST', snr_fig, global_step=self.current_epoch)
        psd_ds_sst, lamb_x_sst, lamb_t_sst = metrics.psd_based_scores(self.test_xr_ds.sst_pred, self.test_xr_ds.sst_gt)
        fig, spatial_res_model_sst, _ = get_psd_score(self.test_xr_ds.ssh_gt, self.test_xr_ds.sst_pred, self.test_xr_ds.sst_pred, with_fig=True)
        self.test_figs['res'] = fig
        self.logger.experiment.add_figure(f'{log_pref} Spat. Resol SST', fig, global_step=self.current_epoch)
        psd_ds_sst, lamb_x_sst, lamb_t_sst = metrics.psd_based_scores(self.test_xr_ds.sst_pred, self.test_xr_ds.sst_gt)
        psd_fig = metrics.plot_psd_score(psd_ds_sst)
        self.test_figs['psd'] = psd_fig
        psd_ds_sst, lamb_x_sst, lamb_t_sst = metrics.psd_based_scores(self.test_xr_ds.sst_pred, self.test_xr_ds.sst_gt)
        self.logger.experiment.add_figure(f'{log_pref} PSD SST', psd_fig, global_step=self.current_epoch)
        ...
        md = {
            f'{log_pref}_spatial_res_ssh': float(spatial_res_model_ssh),
            f'{log_pref}_spatial_res_imp_ssh': float(spatial_res_model_ssh / spatial_res_oi_ssh),
            f'{log_pref}_lambda_x_ssh': lamb_x_ssh,
            f'{log_pref}_lambda_t_ssh': lamb_t_ssh,
            f'{log_pref}_mu_ssh': mu_ssh,
            f'{log_pref}_sigma_ssh': sig_ssh,
            f'{log_pref}_spatial_res_ssh': float(spatial_res_model_sst),
            f'{log_pref}_lambda_x_ssh': lamb_x_sst,
            f'{log_pref}_lambda_t_ssh': lamb_t_sst,
            f'{log_pref}_mu_ssh': mu_sst,
            f'{log_pref}_sigma_ssh': sig_sst,
            **mdf.to_dict(),
        }
    ...
    def diag_epoch_end(self, outputs, log_pref='test'):
        ...
        self.obs_ssh_inp = self.test_xr_ds.ssh_obs.data
        self.x_ssh_oi = self.test_xr_ds.ssh_oi.data
        self.x_ssh_rec = self.test_xr_ds.ssh_pred.data

        self.x_sst_gt = self.test_xr_ds.sst_gt.data
        self.obs_sst_inp = self.test_xr_ds.sst_obs.data
    ...
    def get_init_state(self, batch, state=(None,)):
        ...
        targets_OI, inputs_Mask, inputs_obs, targets_GT, sst_gt, sst_mask = batch
    ...
    def compute_loss(self, batch, phase, state_init=(None,)):
        ...
        sst_gt_wo_nan = targets_GT.where(~sst_gt.isnan(), 0.)
        
        batch = targets_OI, inputs_Mask, inputs_obs, targets_GT, sst_gt, sst_mask
        ...
        new_masks = torch.cat( (new_masks, torch.zeros_like(inputs_Mask), sst_mask), dim=1)
        ...
        with torch.set_grad_enabled(True):
            ...
            if self.aug_state:
                ...
                outputs_sst = outputs[:, 3*self.hparams.dT:, :, :]
            else:
                outputs = outputsSLR + outputs[:, self.hparams.dT:2*self.hparams.dT, :, :]
                outputs_sst = outputs[:, 2*self.hparams.dT:, :, :]
            ...
            if self.median_filter_width > 1:
                ...
                outputs_sst = kornia.filters.median_blur(outputs_sst, (self.median_filter_width, self.median_filter_width))
            ...
            yGT = torch.cat((yGT, sst_gt_wo_nan), dim=1)

            loss_ssh_all, loss_ssh_gall = self.ssh_loss(outputs, targets_GT_wo_nan)
            loss_sst_all, loss_sst_gall = self.ssh_loss(outputs, targets_GT_wo_nan)
            ...
            # total loss
            loss = self.hparams.alpha_mse_ssh * loss_ssh_all + self.hparams.alpha_mse_gssh * loss_ssh_gall
            loss += self.hparams.alpha_mse_sst * loss_sst_all + self.hparams.alpha_mse_gsst * loss_sst_gall
            loss += 0.5 * self.hparams.alpha_proj * (loss_AE + loss_AE_GT)
            loss += self.hparams.alpha_lr * loss_LR + self.hparams.alpha_sr * loss_SR

            # metrics
            # mean_GAll = NN_4DVar.compute_spatio_temp_weighted_loss(g_targets_GT, self.w_loss)
            mean_gssh = NN_4DVar.compute_spatio_temp_weighted_loss(
                    torch.hypot(g_targets_GT_x, g_targets_GT_y) , self.grad_crop(self.patch_weight))
            mean_gsst = NN_4DVar.compute_spatio_temp_weighted_loss(
                    torch.hypot(g_sst_gt_x, g_sst_gt_y) , self.grad_crop(self.patch_weight))
            metrics = dict([
                ('mseSSH', loss_ssh_all.detach()),
                ('mseGSSH', loss_ssh_gall.detach()),
                ('mean_GSSH', mean_gssh),
                ('mean_GSST', mean_gsst),
                ('mseSST', loss_sst_all.detach()),
                ('mseGSST', loss_sst_all.detach())])

        return loss, [outputs,outputs_sst], [outputsSLRHR, hidden_new, cell_new, normgrad], metrics
