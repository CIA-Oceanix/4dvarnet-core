...
class CorrelateNoise(torch.nn.Module):
    ...
    def __init__(self, shape_data, dim_cn):
        ...
        self.conv1 = torch.nn.Conv2d(shape_data,
                                     dim_cn, (3, 3),
                                     padding=1,
                                     bias=False)
        self.conv2 = torch.nn.Conv2d(dim_cn,
                                     2 * dim_cn, (3, 3),
                                     padding=1,
                                     bias=False)
        self.conv3 = torch.nn.Conv2d(2 * dim_cn,
                                     shape_data, (3, 3),
                                     padding=1,
                                     bias=False)
...
class RegularizeVariance(torch.nn.Module):
    ...
    def __init__(self, shape_data, dim_rv):
        ...
        self.conv1 = torch.nn.Conv2d(shape_data,
                                     ...
                                     padding=1,
                                     bias=False)
        self.conv2 = torch.nn.Conv2d(dim_rv,
                                     2 * dim_rv, (3, 3),
                                     padding=1,
                                     bias=False)
        self.conv3 = torch.nn.Conv2d(2 * dim_rv,
                                     shape_data, (3, 3),
                                     padding=1,
                                     bias=False)
...
class ConvLSTM2d(torch.nn.Module):
    ...
    def forward(self, input_, prev_state):
        ...
        if prev_state is None:
            ...
            prev_state = (torch.autograd.Variable(
...
class ConvLSTM1d(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size, kernel_size=3):
        ...
        self.kernel_size = kernel_size
        self.padding = int((kernel_size - 1) / 2)
        self.Gates = torch.nn.Conv1d(input_size + hidden_size,
                                     4 * hidden_size,
                                     kernel_size=self.kernel_size,
                                     stride=1,
                                     padding=self.padding)
...
class model_GradUpdateLSTM(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if len(self.shape) == 2:  # 1D Data
            ...
            self.lstm = ConvLSTM2d(self.shape[0],
                                   self.dim_state,
                                   3,
    ...
    def _make_ConvGrad(self):
        ...
        if len(self.shape) == 2:  # 1D Data
            layers.append(
                torch.nn.Conv1d(self.dim_state,
                                self.shape[0],
                                1,
                                padding=0,
                                bias=False))
            ...
            layers.append(
                torch.nn.Conv2d(self.dim_state,
                                self.shape[0], (1, 1),
                                padding=0,
                                bias=False))
    ...
    def forward(self, hidden, cell, grad, gradnorm=1.0):
        ...
        if self.PeriodicBnd:
            ...
            #
            grad_ = torch.cat(
                (grad[:, :, grad.size(2) - dB:, :], grad, grad[:, :, 0:dB, :]),
                dim=2)
            if hidden is None:
                hidden_, cell_ = self.lstm(grad_, None)
            else:
                hidden_ = torch.cat((hidden[:, :, grad.size(2) - dB:, :],
                                     hidden, hidden[:, :, 0:dB, :]),
                                    dim=2)
                cell_ = torch.cat(
                    (cell[:, :, grad.size(2) - dB:, :], cell, cell[:, :,
                                                                   0:dB, :]),
                    dim=2)
...
class Model_Var_Cost(nn.Module):
    ...
    def __init__(self,
        ...
        self.alphaObs = torch.nn.Parameter(
            torch.Tensor(1. * np.ones((self.dim_obs, 1))))
        self.alphaReg = torch.nn.Parameter(torch.Tensor([1.]))
        if self.dim_obs_channel[0] == 0:
            self.WObs = torch.nn.Parameter(
                torch.Tensor(np.ones((self.dim_obs, ShapeData[0]))))
            self.dim_obs_channel = ShapeData[0] * np.ones((self.dim_obs, ))
        else:
            self.WObs = torch.nn.Parameter(
                torch.Tensor(
                    np.ones((self.dim_obs, np.max(self.dim_obs_channel)))))
        self.WReg = torch.nn.Parameter(torch.Tensor(np.ones(self.dim_state, )))
        self.epsObs = torch.nn.Parameter(
            0.1 * torch.Tensor(np.ones((self.dim_obs, ))))
    ...
    def forward(self, dx, dy):
        ...
        if self.dim_obs == 1:
            loss += self.alphaObs[0]**2 * self.normObs(dy, self.WObs[0, :]**2,
                                                       self.epsObs[0])
        else:
            for kk in range(0, self.dim_obs):
                loss += (self.alphaObs[kk]**2 *
                         self.normObs(dy[kk], self.WObs[kk, 0:dy[kk].size(1)]**
                                      2, self.epsObs[kk]))
...
class Solver_Grad_4DVarNN(nn.Module):
    ...
    def __init__(self,
        ...
        self.model_VarCost = Model_Var_Cost(m_NormObs, m_NormPhi, shape_data,
                                            mod_H.dim_obs,
    ...
    def solve(self, x_0, obs, mask, hidden=None, cell=None, normgrad_=0.):
        ...
        for _ in range(self.n_grad):
            x_k_plus_1, hidden, cell, normgrad_ = self.solver_step(
                x_k, obs, mask, hidden, cell, normgrad_)
    ...
    def solver_step(self, x_k, obs, mask, hidden, cell, normgrad=0.):
        ...
        if normgrad == 0.:
            ...
            normgrad_ = normgrad
        grad, hidden, cell = self.model_Grad(hidden, cell, var_cost_grad,
                                             normgrad_)
        grad *= 1. / self.n_grad
        x_k_plus_1 = x_k - grad
        return x_k_plus_1, hidden, cell, normgrad_
