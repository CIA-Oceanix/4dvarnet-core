...
class Solver_Grad_4DVarNN(nn.Module):
    def __init__(self ,phi_r,mod_H, m_Grad, m_NormObs, m_NormPhi, ShapeData,n_iter_grad,eps=0.,k_step_grad=0.,flag_mr_solver=False,iter_mr_solver=2):
    ...
    def solver_step(self, x_k, obs, mask, hidden, cell,normgrad = 0.,iter=-1):
        ...
        if ( self.flag_mr_solver == True ) & ( iter%self.iter_mr_solver < 1 ) :
            grad = torch.nn.functional.avg_pool2d(grad, (5,5))
            grad = torch.nn.functional.interpolate(grad, scale_factor=5, mode='bicubic')
        x_k_plus_1 = x_k - grad
