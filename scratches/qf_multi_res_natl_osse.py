...
def run1():
    try:
        ...
        dses_ref = {}
        tgt_dx = 1/4
        for tgt_dx, _tgt_dt in [(1, '14D'), (1, '1D'), (1/4, '1D'), ]:
        ...
        xr.Dataset({k: v.ssh for k, v in dses_obs.items()}).to_netcdf('../sla-data-registry/natl60_degraded/obs220914.nc')
        xr.Dataset({k: v.ssh for k, v in dses_ref.items()}).to_netcdf('../sla-data-registry/natl60_degraded/ref220914.nc')
...
def fmt_orca25():
    try:
        ...
        new_obs_ds = sample_nadirs_from_simu(nadirs, fmtted_orca)
