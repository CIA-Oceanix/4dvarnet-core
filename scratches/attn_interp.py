...
@dataclass
class XpConfig:
    obs_file: str = '../research-quentin/data/osse_gulfstream/GULFSTREAM/data/gridded_data_swot_wocorr/dataset_nadir_0d_swot.nc'
    oi_file: str = '../research-quentin/data/osse_gulfstream/GULFSTREAM/oi/ssh_NATL60_swot_4nadir.nc'
    ref_file: str = '../research-quentin/data/osse_gulfstream/GULFSTREAM/ref/NATL60-CJM165_GULFSTREAM_ssh_y2013.1y.nc'
...
class LatentMeshInterpolator(nn.Module):
    def __init__(self, nmesh, n_embd, *args, **kwargs):
        super().__init__()
        self.nmesh = nmesh
        self.mesh_coords = nn.Parameter(torch.rand(nmesh, 3))
...
class LitInterp(pl.LightningModule):
    def __init__(self, mod, cfg, oi_ds=None):
    ...
    def forward(self, coords):
        return self.mod(coords)

    def training_step(self, batch, batch_idx):
        tgt_coords, tgt_values = batch
        tgt_pred = self.mod(tgt_coords, mesh_sample_prop=0.9)
    ...
    def configure_optimizers(self, ):
        opt = torch.optim.Adam(self.parameters(), lr=0.01)  # , weight_decay=0.0001)
        return {
            'optimizer': opt,
...
def main(cfg):
    ...
    oi_ds = xr.open_dataset(cfg.xp.oi_file)
    ...
    test_slice = slice(*cfg.xp.train_slice)
    train_ds = InterpDataset(obs_ds.isel(time=train_slice), data_vars=('ssh_mod',))
    test_ds = InterpDataset(ref_ds.isel(time=test_slice), coord_norm=train_ds.coord_norm)

    train_dl = DataLoader(train_ds, batch_size=cfg.xp.bs, shuffle=True)
    test_dl = DataLoader(test_ds, batch_size=cfg.xp.bs)
    ...
    # %% training
    lit_mod = LitInterp(mod=mod, cfg=cfg.xp, oi_ds=oi_ds.isel(time=test_slice))
