...
import numpy as np
N=${N}
data = xr.open_dataset('dashboard/oi_osse_spde_qg/lightning_logs/version_'+str(N)+'/maps.nc')
for i in range(1,2):
    new_simu = xr.open_dataset('dashboard/oi_osse_spde_qg/lightning_logs/version_'+str(N+i)+'/maps.nc')
...
for (( i=1; i<$len; i++ )); do
  ...
  for (( j=0; j<2; j++ )); do
    ...
    CUDA_VISIBLE_DEVICES=4 HYDRA_FULL_ERROR=1 python hydra_main.py xp=mbeaucha/xp_spde/ssh/oi_osse_spde file_paths=mee_a100_ssh entrypoint=test datamodule.test_slices.0._args_.0='"'$start'"' datamodule.test_slices.0._args_.1='"'$end'"' +entrypoint.ckpt_path=../../model_oi_osse_spde_qg.ckpt
  done
  # merge NetCDF
...
nc.to_netcdf('/homes/m19beauc/maps_osse_4DvarNet_SPDE_qg.nc') 
HEREDOC  
