...
from oi.xp_gp.solver_spde import Model_Var_Cost
...
class LitModel(pl.LightningModule):
    ...
    def test_epoch_end(self, outputs):
        ...
        kappa = 1./3
        m = torch.cat([(chunk['params'])[0].detach().cpu() for chunk in outputs]).numpy()
        H = None
    ...
    def compute_loss(self, batch, phase):
        ...
        with torch.set_grad_enabled(True):
            ...
            n_b = inputs_init.shape[0]
            n_x = inputs_init.shape[3]
            n_y = inputs_init.shape[2]
            nb_nodes = n_x*n_y
            n_t = inputs_init.shape[1]

            outputs, params = self.model(inputs_init, inputs_missing, inputs_mask,
                                         .33,
                                         torch.stack((
                                              torch.permute(torch.reshape(targets_u,(n_b,n_t,nb_nodes)),(0,2,1)),
                                              torch.permute(torch.reshape(targets_v,(n_b,n_t,nb_nodes)),(0,2,1))),
                                              dim=1),
                                         None)
            ...
            dy = 1
            dt = 1
            m = torch.reshape(params[0],(n_b,2,n_x*n_y,n_t))
            H = None
            kappa = 1./3
            tau = 1.
