...
class Solver_Grad_4DVarNN(nn.Module):
    ...
    def solver_step(self, state_k, obs, mask, hidden, cell,normgrad = 0.,estim_parameters=True):
        ...
        state_k_plus_1 = torch.index_add(state_k_plus_1,1,torch.arange(n_t,3*n_t).to(device),-1*state_k_plus_1[:,n_t:,:,:])
        state_k_plus_1 = torch.index_add(state_k_plus_1,1,torch.arange(n_t,3*n_t).to(device),aug_var)
