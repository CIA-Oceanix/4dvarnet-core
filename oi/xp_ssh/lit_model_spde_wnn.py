...
import oi.xp_ssh.solver as NN_4DVar2
from oi.xp_ssh.models import Phi_r as Phi_r_nn
from oi.xp_ssh.lit_model import LitModel as lm_nn
...
class LitModel(pl.LightningModule):
    def __init__(self, hparam, *args, **kwargs):
        ...
        self.shapeData_nn = [2*self.hparams.dT,int(self.hparams.shapeData[1]/self.hparams.resize_factor),
                                          int(self.hparams.shapeData[2]/self.hparams.resize_factor)]
        ...
        self.model = NN_4DVar.Solver_Grad_4DVarNN(
                ...
                NN_4DVar.model_GradUpdateLSTM([9*self.hparams.dT,self.Ny,self.Nx],
        ...
        self.model_nn = lm_nn.load_from_checkpoint("../../model_oi_osse.ckpt",
                                                    #hparam=self.cfg,
                                                    hparam=hparams,
                                                    w_loss=self.w_loss,
                                                    strict=False,
                                                    ...
                                                    min_lon=self.xmin, max_lon=self.xmax,
                                                    min_lat=self.ymin, max_lat=self.ymax,
                                                    ...
                                                    test_domain=hparams['test_domain'],
    ...
    def forward(self, batch, phase='test'):
        ...
        if self.use_oi:
            ...
            batch = targets_oi, inputs_mask, inputs_obs, targets_gt
    ...
    def apply_model_nn(self, batch):

        targets_oi, inputs_mask, inputs_obs, targets_gt = batch
        _, out, _ = self.model_nn(batch, phase='test')
        return out
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        with torch.inference_mode(mode=False):
            ...
            out = out[:,:n_t,:,:]
            out = self.apply_model_nn(test_batch)
            #if self.use_oi:
            #    out = out + targets_oi
            ...
            nb_nodes = n_x*n_y
            dx, dy, dt = [1,1,1]
            kappa = torch.reshape(param[0],(n_b,1,nb_nodes,n_t))
            m = torch.reshape(param[1],(n_b,2,nb_nodes,n_t))
            H = torch.reshape(param[2],(n_b,2,2,nb_nodes,n_t))
            tau = torch.reshape(param[3],(n_b,1,nb_nodes,n_t))
            ...
            for isimu in range(self.n_simu):
                ...
                simu_batch = targets_oi, inputs_mask, inputs_obs_simu+torch.where(inputs_mask!=0, targets_oi, torch.zeros_like(targets_oi)), x_simu+targets_oi
                #_, x_itrp_simu, _, _ = self(simu_batch, phase='test')
                x_itrp_simu = self.apply_model_nn(simu_batch)-targets_oi
                x_itrp_simu = x_itrp_simu.detach()
                # conditional simulation
                #x_simu_cond = x_itrp_simu
                #x_simu_cond = x_itrp_simu - x_simu
    ...
    def compute_loss(self, batch, phase, state_init=(None,)):
        ...
        if state_init[0] is not None:
            ...
            with torch.set_grad_enabled(True):
                state_init = torch.autograd.Variable(inputs_init, requires_grad=True)
                # à mettre avant le state_init (crée une initialisation de l'état avec 4DVarNet classique)
                outputs_4dvarnet = self.apply_model_nn(batch)
            state_init = outputs_4dvarnet.clone()
            inputs_obs = inputs_obs.clone()
            inputs_mask = inputs_mask.clone()
            state_init = (targets_gt-targets_oi).clone()
            inputs_obs = (targets_gt-targets_oi).clone()
            inputs_mask = torch.ones_like(targets_gt).to(device)

            # augmented state
            kappa_init = torch.ones(n_b,n_t,n_x,n_y).to(device)*0.1
            tau_init = torch.ones(n_b,n_t,n_x,n_y).to(device)*0.1
            m1_init = torch.ones(n_b,n_t,n_x,n_y).to(device)*0.1
            m2_init = torch.ones(n_b,n_t,n_x,n_y).to(device)*0.1
            vx_init = torch.zeros(n_b,n_t,n_x,n_y).to(device)
            vy_init = torch.zeros(n_b,n_t,n_x,n_y).to(device)
            gamma_init = torch.ones(n_b,n_t,n_x,n_y).to(device)
            beta_init = torch.zeros(n_b,n_t,n_x,n_y).to(device)
        ...
        #with torch.set_grad_enabled(True), torch.autograd.detect_anomaly():
            ...
            if phase=='test':
                ...
                #tau[:,:,2500:5000,:] = 0.1
                #tau[:,:,:,:] = 1
                #kappa = torch.reshape(torch.nn.functional.relu(torch.permute(targets_gt-targets_oi,(0,3,2,1)))*1,tau.shape) + .01
                #kappa = (1./kappa)/100
                #tau = torch.reshape(torch.nn.functional.relu(torch.permute(targets_gt-targets_oi,(0,3,2,1)))*1+.1,tau.shape)
                #tau = 1./tau
            '''
            ...
            if self.loss_type=="ml_loss":
                ...
                for i in range(n_b):
                    ...
                    for j in range(0,len(block_diag[i])):
                        chol_block = torch.cholesky(block_diag[i][j].to_dense())
                ...
                print(10*loss_All)
                print(loss_NLL*(1e-6))
                loss = loss_NLL*(1e-6) + 10*loss_All #+ loss_GAll # (ou loss OI ?)
