...
def psd_based_scores(ds,var_pred,dx,dt):
    ...
    with ProgressBar():
        ...
        logging.info('          => Leaderboard Spectral score = %s (x-axis unit='+str(dx)+')',
                     np.round(shortest_spatial_wavelength_resolved, 2))
        logging.info('          => shortest temporal wavelength resolved = %s (algorithmic time dt='+str(dt)+')',
...
def plot_psd_score(ds_psd,dx,dt):
    ...
    for exp in range(nb_experiment):
        ...
        ax.set_xlabel('spatial wavelength (x-axis unit='+str(dx)+')', fontweight='bold', fontsize=18)
        ax.set_ylabel('temporal wavelength (algorithmic time dt='+str(dt)+')', fontweight='bold', fontsize=18)
        #plt.xscale('log')
        ax.set_yscale('log')
...
def TS(list_data,labels_data,colors,symbols,lstyle,lwidth,dt,gradient=False):
    ...
    plt.ylabel('nRMSE')
    plt.xlabel('Time (algorithmic time dt='+str(dt)+')')
    plt.margins(x=0)
