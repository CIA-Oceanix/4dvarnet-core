...
class Solver_Grad_4DVarNN(nn.Module):
    ...
    def forward(self, gt, params_gt, state, obs, mask, oi, estim_parameters=True):
        return self.solve(
            gt=gt,
            params_gt=params_gt,
            state_init=state,
    ...
    def solve(self, gt, params_gt, state_init, obs, mask, oi, estim_parameters):
        ...
        for k in range(self.n_grad):
            state_k_plus_1, cmp_loss_, hidden, cell, normgrad_, params = self.solver_step(gt, params_gt, state_k, obs, mask, oi,
    ...
    def solver_step(self, gt, params_gt, state_k, obs, mask, oi, hidden, cell,normgrad = 0.,estim_parameters=True):
        _, cmp_loss, var_cost_grad, params = self.var_cost(gt, params_gt, state_k, obs, mask, oi, estim_parameters)
        if normgrad == 0. :
    ...
    def var_cost(self, gt, params_gt, state, yobs, mask, oi, estim_params):
