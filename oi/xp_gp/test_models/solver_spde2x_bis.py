...
class ConvLSTM1d(torch.nn.Module):
    ...
    def forward(self, input_, prev_state):
        ...
        # data size is [batch, channel, height, width]
        stacked_inputs = torch.cat((torch.unsqueeze(input_,dim=1), prev_hidden), 1)
        gates = self.Gates(stacked_inputs)
...
class model_GradUpdateLSTM(torch.nn.Module):
    def __init__(self,ShapeData,periodicBnd=False,DimLSTM=0,rateDropout=0.,stochastic=False):
        ...
        if len(self.shape) == 1: ## 1D Data
            self.lstm = ConvLSTM1d(1,self.dim_state,3)
    ...
    def _make_ConvGrad(self):
        layers = []
        if len(self.shape) == 1: ## 1D Data
            layers.append(torch.nn.Conv1d(self.dim_state, 1, 1, padding=0,bias=False))
        elif len(self.shape) == 3: ## 2D Data
    ...
    def _make_LSTMGrad(self):
        ...
        if len(self.shape) == 1: ## 1D Data
            ...
            #layers.append(ConvLSTM1d(self.shape[0],self.shape[0],3))
        elif len(self.shape) == 3: ## 2D Data
...
class Solver_Grad_4DVarNN(nn.Module):
    ...
    def __init__(self ,phi_r,mod_H, m_Grad, m_Grad_theta, m_NormObs, m_NormPhi, shape_data,n_iter_grad,
                 square_root=False):
        ...
        self.model_Grad_theta = m_Grad_theta
    ...
    def forward(self, gt, params_gt, state, coeff_theta, obs, mask, oi, estim_parameters=True):
        return self.solve(
            gt=gt,
            params_gt=params_gt,
            state_init=state,
            coeff_theta_init=coeff_theta,
            obs=obs,
            mask = mask,
            oi = oi,
            estim_parameters = estim_parameters)

    def solve(self, gt, params_gt, state_init, coeff_theta_init, obs, mask, oi, estim_parameters):
        n_b, n_t, n_x, n_y = obs.shape
        state_k = torch.mul(state_init,1.) 
        coeff_theta_k = torch.mul(coeff_theta_init,1.)
        hidden = None
        cell = None 
        normgrad_ = 0.
        hidden_theta = [ None for _ in range(coeff_theta_k.shape[2]) ]
        cell_theta = [ None  for _ in range(coeff_theta_k.shape[2]) ]
        normgrad_theta_ = torch.zeros(coeff_theta_k.shape[2])
        x_k_plus_1 = None
        if oi is None:
            cmp_loss = torch.ones(state_k.shape[0],self.n_grad,2)
        else:
            cmp_loss = torch.ones(state_k.shape[0],self.n_grad,3)        
        for k in range(self.n_grad):
            state_k_plus_1, coeff_theta_k_plus_1, cmp_loss_, \
                    hidden, cell, normgrad_,\
                    hidden_theta, cell_theta, normgrad_theta_,\
                    params = self.solver_step(gt, 
                                            params_gt,
                                            state_k, 
                                            coeff_theta_k, 
                                            obs, 
                                            ...
                                            hidden, hidden_theta, 
                                            cell, cell_theta,
                                            normgrad_, normgrad_theta_,
                                            estim_parameters)
            state_k = torch.mul(state_k_plus_1,1.)
            coeff_theta_k = torch.mul(coeff_theta_k_plus_1,1.)
            for batch in range(len(cmp_loss)):
                cmp_loss[batch,k,:] = cmp_loss_[batch,:]
        
        print(coeff_theta_k_plus_1)

        return state_k_plus_1[:,:n_t,:,:], cmp_loss, hidden, cell, normgrad_, params

    def solver_step(self, gt, params_gt, state_k, coeff_theta_k, obs, mask, oi, 
                    hidden, hidden_theta,
                    cell, cell_theta, 
                    normgrad = 0., normgrad_theta = 0.,
                    estim_parameters=True):
        _, cmp_loss, var_cost_grad_x, var_cost_grad_theta, params = self.var_cost(gt, 
                                                                                  params_gt,
                                                                                  state_k,
                                                                                  coeff_theta_k,
                                                                                  obs,
                                                                                  ...
                                                                                  estim_parameters)
        if normgrad == 0. :
            normgrad_= torch.sqrt( torch.mean( var_cost_grad_x**2 + 0.))
        else:
            normgrad_= normgrad
        if torch.count_nonzero(normgrad_theta) == 0. :
            normgrad_theta_ = torch.Tensor([ torch.sqrt( torch.mean( var_cost_grad_theta[:,:,i]**2 + 0.)) for i in range(coeff_theta_k.shape[2]) ]).to(device)
        else:
            normgrad_theta_ = normgrad_theta
        grad, hidden, cell = self.model_Grad(hidden, cell, var_cost_grad_x, normgrad_)
        grad *= 1./ self.n_grad
        state_k_plus_1 = state_k - grad
        # loop on the Fourier coefficients: set of lstm1d
        coeff_theta_k_plus_1 = []
        for i in range(coeff_theta_k.shape[2]):
            grad_theta_i, hidden_theta[i], cell_theta[i] = self.model_Grad_theta[i](hidden_theta[i],
                                                                                    cell_theta[i],
                                                                                    var_cost_grad_theta[:,:,i],
                                                                                    normgrad_theta_[i])
            grad_theta_i *= 1./ self.n_grad
            coeff_theta_k_plus_1.append(coeff_theta_k[:,:,i] - grad_theta_i)
        coeff_theta_k_plus_1 = torch.stack(coeff_theta_k_plus_1,dim=2)
        return state_k_plus_1, coeff_theta_k_plus_1, cmp_loss,  hidden, cell, normgrad_, hidden_theta, cell_theta, normgrad_theta_, params

    def var_cost(self, gt, params_gt, state, coeff_theta, yobs, mask, oi, estim_params):
        n_b, n_t, n_x, n_y = state.shape
        # augmented state [x,coeff]
        with torch.set_grad_enabled(True):
            ...
            if self.square_root==True:
                ...
                phi = self.phi_r([x,coeff_theta],estim_params=estim_params)
                dx = phi[0]
                params = phi[1]
                loss_OI = dy + dx 
                loss = torch.mean(loss_OI)
            #loss = torch.mean(params_gt-params[2])
            #var_cost_grad_x = torch.zeros(x.shape).to(device)
            var_cost_grad_x = torch.autograd.grad(loss, x, create_graph=True)[0]
            var_cost_grad_theta = torch.autograd.grad(loss, coeff_theta, create_graph=True)[0]
        ...
        if oi is not None:
            ...
            return loss, torch.hstack([torch.reshape(loss_mse,(n_b,1)), torch.reshape(loss_OI,(n_b,1)), ]), var_cost_grad_x, var_cost_grad_theta, params
