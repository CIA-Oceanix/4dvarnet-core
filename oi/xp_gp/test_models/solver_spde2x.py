...
class Solver_Grad_4DVarNN(nn.Module):
    ...
    def forward(self, gt, state, coeff_theta, obs, mask, oi, estim_parameters=True):
        return self.solve(
            gt=gt,
            state_init=state,
    ...
    def solve(self, gt, state_init, coeff_theta_init, obs, mask, oi, estim_parameters):
    ...
    def solver_step(self, gt, state_k, coeff_theta_k, obs, mask, oi, 
                    hidden, hidden_theta,
                    cell, cell_theta, 
        ...
        if normgrad == 0. :
            ...
            normgrad_= normgrad
        if normgrad_theta == 0. :
            normgrad_theta_ = torch.sqrt( torch.mean( var_cost_grad_theta**2 + 0.))
        else:
            normgrad_theta_ = normgrad_theta
        grad, hidden, cell = self.model_Grad(hidden, cell, var_cost_grad_x, normgrad_)
        #grad_theta, hidden_theta, cell_theta = self.model_Grad_theta(hidden_theta, cell_theta, var_cost_grad_theta, normgrad_theta_)
        grad_theta = 0.0001*var_cost_grad_theta
        grad *= 1./ self.n_grad
        grad_theta *= 1./ self.n_grad
        state_k_plus_1 = state_k - grad
        coeff_theta_k_plus_1 = coeff_theta_k - grad_theta
        return state_k_plus_1, coeff_theta_k_plus_1, cmp_loss,  hidden, cell, normgrad_, hidden_theta, cell_theta, normgrad_theta_, params

    def var_cost(self, gt, state, coeff_theta, yobs, mask, oi, estim_params):
