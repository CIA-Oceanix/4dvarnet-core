...
class LitModel(pl.LightningModule):
    def __init__(self, hparam, *args, **kwargs):
        ...
        self.model = NN_4DVar.Solver_Grad_4DVarNN(
                ...
                [NN_4DVar.model_GradUpdateLSTM([self.hparams.dT],
                                               self.hparams.UsePriodicBoundary,
                                               self.hparams.dim_grad_solver,
                                               self.hparams.dropout, self.hparams.stochastic).to(device) for i in range(self.n_params)],
    ...
    def configure_optimizers(self):
        ...
        optimizer = optim.Adam([{'params': self.model.model_Grad.parameters(), 'lr': self.hparams.lr_update[0]},
                                {'params': torch.nn.ModuleList(self.model.model_Grad_theta).parameters(), 'lr': self.hparams.lr_update[0]},
    ...
    def compute_loss(self, batch, phase):
        ...
        coeff_fourier_init = 0.5*torch.ones((n_b,n_t,self.n_params)).to(device)
        coeff_fourier_init[:,:,1] = 25
        coeff_fourier_init[:,:,0] = 1
        ...
        '''
        coeff_fourier_false = 0.8*torch.ones(n_b,n_t,self.n_params).to(device)
        #coeff_fourier_false[:,:,:10] = 0.2
        coeff_fourier_false[:,:,0] = 1
        coeff_fourier_false[:,:,1] = 25
        params_gt = self.model.phi_r.decoder(coeff_fourier_false[:,n_t//2,:])
        params_gt = torch.reshape(params_gt,(n_b,2,2,n_x,n_y))
        H11_gt = params_gt[:,0,0,:,:]
        H12_gt = params_gt[:,0,1,:,:]
        H22_gt = params_gt[:,1,1,:,:]
        '''
        params_gt = torch.zeros(n_b,2,2,100,100).to(device)
        params_gt[:,0,0,:,:] = torch.squeeze(H11_gt)
        params_gt[:,0,1,:,:] = torch.squeeze(H12_gt)
        params_gt[:,1,0,:,:] = torch.squeeze(H12_gt)
        params_gt[:,1,1,:,:] = torch.squeeze(H22_gt)
        '''
        ...
        with torch.set_grad_enabled(True):
            if phase=='test':
                ...
                coeff_fourier_init = coeff_fourier_init.clone()
            with torch.inference_mode(False):
            ...
            if self.loss_type=="diffusion_loss":
                ...
                '''
                '''
                H11_gt = (H11_gt-torch.mean(H11_gt))/torch.std(H11_gt)
                H12_gt = (H12_gt-torch.mean(H12_gt))/torch.std(H12_gt)
                H22_gt = (H22_gt-torch.mean(H22_gt))/torch.std(H22_gt)
                '''
