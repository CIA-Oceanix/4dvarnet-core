...
def interpolate_na_2D(da, max_value=100.):
    return (da.where(
        np.abs(da) < max_value,
        np.nan).pipe(lambda da: da).to_dataframe().interpolate().pipe(
...
class XrDataset(Dataset):
    ...
    def __init__(
        ...
        rename_coords = {}
        if "lat" not in _ds.coords and "latitude" in _ds.coords:
            rename_coords["latitude"] = "lat"
        if "lon" not in _ds.coords and "longitude" in _ds.coords:
            rename_coords["longitude"] = "lon"
        _ds = _ds.rename(rename_coords)

        self.ds = _ds.sel(**(dim_range or {}))
        if resize_factor != 1:
            self.ds = self.ds.coarsen(lon=resize_factor).mean(
                skipna=True).coarsen(lat=resize_factor).mean(skipna=True)
        ...
        if self.auto_padding:
            # dimensions
            self.Nt, self.Nx, self.Ny = tuple(self.ds.dims[d]
                                              for d in ['time', 'lon', 'lat'])
            ...
            dim_range_ = {
                'lon':
                slice(self.ds.lon.min().item() - dX[0],
                      self.ds.lon.max().item() + dX[1]),
                'lat':
                slice(self.ds.lat.min().item() - dY[0],
                      self.ds.lat.max().item() + dY[1]),
                'time':
                dim_range['time']
            }
            ...
            dX = [pad_ * self.resolution for pad_ in pad_x]
            dY = [pad_ * self.resolution for pad_ in pad_y]
            pad_ = {'lon': (pad_x[0], pad_x[1]), 'lat': (pad_y[0], pad_y[1])}
            ...
            self.padded_coords = {
                c: self.ds[c].pad(pad_,
                                  end_values=end_coords,
                                  mode="linear_ramp")
            ...
            dim_range_ = {
                'lon':
                slice(dim_range_['lon'].start + dX,
                      dim_range_['lon'].stop - dX),
                'lat':
                slice(dim_range_['lat'].start + dY,
                      dim_range_['lat'].stop - dY),
            }
        ...
        self.ds_size = {
            dim: max((self.ds.dims[dim] - slice_win[dim]) //
...
class FourDVarNetDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if item < length:
            ...
            _obs_item = np.where(obs_mask_item, _gt_item,
                                 np.full_like(_gt_item, np.nan))
        ...
        obs_item = np.where(~np.isnan(_obs_item), _obs_item,
                            np.zeros_like(_obs_item))

        if self.sst_ds is None:
...
class FourDVarNetDataModule(pl.LightningDataModule):
    ...
    def __init__(
            ...
            dim_range=None,
            strides=None,
            train_slices=(slice('2012-10-01',
                                "2012-11-20"), slice('2013-02-07',
                                                     "2013-09-30")),
    ...
    def mean_stds(self, ds):
        ...
        if self.sst_var is None:
            ...
            mean_sst = float(
                xr.concat(
                    [_ds.sst_ds.ds[_ds.sst_ds.var] for _ds in ds.datasets],
                    dim='time').mean())
            std_sst = float(
                xr.concat(
                    [_ds.sst_ds.ds[_ds.sst_ds.var] for _ds in ds.datasets],
    ...
    def get_domain_bounds(self, ds):
        min_lon = round(
            np.min(
                np.concatenate(
                    [_ds.gt_ds.ds['lon'].values for _ds in ds.datasets])), 2)
        max_lon = round(
            np.max(
                np.concatenate(
                    [_ds.gt_ds.ds['lon'].values for _ds in ds.datasets])), 2)
        min_lat = round(
            np.min(
                np.concatenate(
                    [_ds.gt_ds.ds['lat'].values for _ds in ds.datasets])), 2)
        max_lat = round(
            np.max(
                np.concatenate(
                    [_ds.gt_ds.ds['lat'].values for _ds in ds.datasets])), 2)
    ...
    def setup(self, stage=None):
        ...
        if self.sst_var is None:
            ...
            self.norm_stats, self.norm_stats_sst = self.compute_norm_stats(
                self.train_ds)
            self.set_norm_stats(self.train_ds, self.norm_stats,
                                self.norm_stats_sst)
            self.set_norm_stats(self.val_ds, self.norm_stats,
...
if __name__ == '__main__':
    ...
    dm = get_dm(
        'xp_aug/xp_repro/full_core_sst',
        add_overrides=['datamodule.sst_path=${file_paths.natl_sst_daily}'])
    ...
    ds.obs_mask_ds.ds.ssh_mod.pipe(np.isfinite).mean('time').isel(
        lat=slice(20, -20), lon=slice(20, -20)).plot()
