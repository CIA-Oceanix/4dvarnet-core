...
class Encoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_tr = torch.nn.ConvTranspose2d(dim_out,
                                                dim_out, (ss, ss),
                                                stride=(ss, ss),
                                                bias=False)
        ...
        self.nn_lr = self.__make_BilinNN(dim_inp, dim_out, self.dim_ae, dw,
                                         dw2, self.nb_blocks, rateDropout)
        self.nn_hr = self.__make_BilinNN(dim_inp, dim_out, self.dim_ae, dw,
                                         dw2, self.nb_blocks, rateDropout)
        self.dropout = torch.nn.Dropout(rateDropout)

    def __make_BilinNN(self,
                       dim_inp,
                       dim_out,
                       dim_ae,
                       dw,
...
class Phi_r(torch.nn.Module):
    ...
    def forward(self, x):
        if self.stochastic:
            # pure white noise
            z = torch.randn([x.shape[0], x.shape[1], x.shape[2],
                             x.shape[3]]).to(device)
...
class Model_HwithSST(torch.nn.Module):
    ...
    def __init__(self, shape_data, dT=5, dim=5):
        ...
        self.conv11 = torch.nn.Conv2d(shape_data,
                                      self.dim_obs_channel[1], (3, 3),
                                      padding=1,
                                      bias=False)
        self.conv21 = torch.nn.Conv2d(dT,
                                      self.dim_obs_channel[1], (3, 3),
                                      padding=1,
                                      bias=False)
        self.conv_m = torch.nn.Conv2d(dT,
                                      self.dim_obs_channel[1], (3, 3),
                                      padding=1,
                                      bias=False)
...
class Gradient_img(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_gx = torch.nn.Conv2d(1,
                                       1,
                                       kernel_size=3,
                                       stride=1,
                                       padding=1,
                                       bias=False)
        self.conv_gx.weight = torch.nn.Parameter(
            torch.from_numpy(a).float().unsqueeze(0).unsqueeze(0),
            requires_grad=False)

        b = np.array([[1., 2., 1.], [0., 0., 0.], [-1., -2., -1.]])
        self.conv_gy = torch.nn.Conv2d(1,
                                       1,
                                       kernel_size=3,
                                       stride=1,
                                       padding=1,
                                       bias=False)
        self.conv_gy.weight = torch.nn.Parameter(
    ...
    def forward(self, im):
        ...
        if im.size(1) == 1:
            ...
            g_y = self.conv_gy(im)
            g = torch.sqrt(
                torch.pow(0.5 * g_x, 2) + torch.pow(0.5 * g_y, 2) + self.eps)
        else:

            for kk in range(0, im.size(1)):
                g_x = self.conv_gx(im[:,
                                      kk, :, :].view(-1, 1, im.size(2),
                                                     im.size(3)))
                g_y = self.conv_gy(im[:,
                                      kk, :, :].view(-1, 1, im.size(2),
                                                     im.size(3)))
                ...
                ng = torch.sqrt(
                    torch.pow(0.5 * g_x, 2) + torch.pow(0.5 * g_y, 2) +
                    self.eps)
