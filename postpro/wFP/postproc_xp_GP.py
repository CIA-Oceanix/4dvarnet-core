...
for ipatch in range(19):   
    ...
    # plot MSE loss vs OI loss (train MSE OI)
    oi_loss_oi = find_open_ncdf_loss("oi_gp_ref_"+gp_dataset).loss_oi.isel(patch=ipatch)
    oi_loss_mse = find_open_ncdf_loss("oi_gp_ref_"+gp_dataset).loss_mse.isel(patch=ipatch)
    # OI (gradient descent)
    oi_gd_loss_oi = find_open_ncdf_loss("oi_gp_ref_gd_"+gp_dataset).loss_oi.isel(patch=ipatch)
    ...
    p3 = plt.plot(unet_mseoi_loss_loss_oi, unet_mseoi_loss_loss_mse, '-o', c='slategray', alpha=0.5, label='UNet (mse loss)')
    p4 = plt.plot(unet_oi_loss_loss_oi, unet_oi_loss_loss_mse, 's--', c='slategray', alpha=0.5, label='UNet (oi loss)')

    p5 = plt.plot(fourdvarnet_fp_phi_unet_mseoi_loss_loss_oi, fourdvarnet_fp_phi_unet_mseoi_loss_loss_mse, '-o', c='darkorange', alpha=0.5,label='Prior: UNet / Solver: FP10 (mse loss)')
    p6 = plt.plot(fourdvarnet_fp_phi_unet_oi_loss_loss_oi, fourdvarnet_fp_phi_unet_oi_loss_loss_mse, 's--', c='darkorange', alpha=0.5,label='Prior: UNet / Solver: FP10 (oi loss)')

    p7 = plt.plot(fourdvarnet_lstm_phi_unet_mseoi_loss_loss_oi, fourdvarnet_lstm_phi_unet_mseoi_loss_loss_mse, '-o', c='darkslateblue', alpha=0.5,label='Prior: UNet / Solver: LSTM (20 updates) (mse loss)')
    p8 = plt.plot(fourdvarnet_lstm_phi_unet_oi_loss_loss_oi, fourdvarnet_lstm_phi_unet_oi_loss_loss_mse, 's--', c='darkslateblue', alpha=0.5,label='Prior: UNet / Solver: LSTM (100 updates) (oi loss)')

    p9 = plt.plot(fourdvarnet_lstm_phi_cov_mseoi_loss_loss_oi, fourdvarnet_lstm_phi_cov_mseoi_loss_loss_mse, '-o', c='firebrick', alpha=0.5,label='Prior: Covariance / Solver: LSTM (20 updates) (mse loss)')
    p10 = plt.plot(fourdvarnet_lstm_phi_cov_oi_loss_loss_oi, fourdvarnet_lstm_phi_cov_oi_loss_loss_mse, 's--', c='firebrick', alpha=0.5,label='Prior: Covariance / Solver: LSTM (100 updates) (oi loss)')

    ymin = min(np.concatenate((oi_loss_mse.values,
           fourdvarnet_lstm_phi_unet_mseoi_loss_loss_mse.values,
           fourdvarnet_lstm_phi_cov_mseoi_loss_loss_mse.values)))
    ymax = max(np.concatenate((oi_loss_mse.values,
           fourdvarnet_lstm_phi_unet_mseoi_loss_loss_mse.values,
           fourdvarnet_lstm_phi_cov_mseoi_loss_loss_mse.values)))
    xmin = min(np.concatenate((oi_loss_oi.values,
    ...
    plt.legend([handles[idx] for idx in order],[labels[idx] for idx in order],
              loc='upper center', bbox_to_anchor=(0.5, -0.1), ncol=2)
    plt.savefig('4DVarNet_xp_GP_'+gp_dataset+'_loss2_'+str(ipatch)+'.png', bbox_inches='tight')
    fig = plt.gcf()
    plt.close()
