...
genSuffixModel = genSuffixModel + '_Grad_' + str('%02d' % (GradType)) + '_' + str('%02d' % (OptimType)) + '_' + str(
    '%02d' % (NBGradCurrent)) + '_' + str('%02d' % (dimGradSolver))
    ...
    def compute_loss(self, batch, phase):
        ...
        with torch.set_grad_enabled(True):
            ...
            if flagMultiScale == True:
                ...
                loss_Sampling2 = torch.mean(
                    1. - torch.nn.functional.threshold(1. - new_masks[idxSampMat][:, dT:, :, :], 0.9, 0.))
