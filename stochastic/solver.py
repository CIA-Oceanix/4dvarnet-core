...
class Model_Var_Cost(nn.Module):
    ...
    def forward(self, dx, dy):
        ...
        loss_prior = self.alphaReg**2 * self.normPrior(dx,self.WReg**2,self.epsReg)
        if self.dim_obs == 1 :
            loss_obs =  self.alphaObs[0]**2 * self.normObs(dy,self.WObs[0,:]**2,self.epsObs[0])
        else:
        ...
        loss = loss_prior + loss_obs
        return loss, loss_prior, loss_obs
...
class Solver_Grad_4DVarNN(nn.Module):
    ...
    def solve(self, x_0, obs, mask, hidden=None, cell=None, normgrad_=0.):
        ...
        for _ in range(self.n_grad):
            dy = self.model_H(x_k,obs,mask)
            dx = x_k - self.phi_r(x_k)
            x_k_plus_1, hidden, cell, normgrad_ = self.solver_step(x_k, dy, dx, hidden, cell, normgrad_)
            x_k = torch.mul(x_k_plus_1,1.)
        return x_k_plus_1, hidden, cell, normgrad_

    def solver_step(self, x_k, dy, dx, hidden, cell,normgrad = 0.):
        _, var_cost_grad= self.var_cost(x_k, dy, dx)
        if normgrad == 0. :
    ...
    def var_cost(self, x, dy, dx):
        loss, _, _ = self.model_VarCost( dx , dy )
